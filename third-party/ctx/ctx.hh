...
namespace ctx {
  ...
  fcontext_t create_ctx1(fixedsize_stack&& salloc, Fn&& fn) {
  ...
  template< typename Record, typename Fn >
  fcontext_t create_ctx2(preallocated palloc, fixedsize_stack&& salloc, Fn&& fn) {
	// reserve space for control structure
	void* storage = reinterpret_cast<void*>(
  ...
  public:
	...
	continuation(std::allocator_arg_t, fixedsize_stack&& salloc, Fn&& fn) :
	  fctx_{ create_ctx1< record< continuation, Fn > >(
	...
	continuation(std::allocator_arg_t, preallocated palloc, fixedsize_stack&& salloc, Fn&& fn) :
	  fctx_{ create_ctx2< record< continuation, Fn > >(
	...
	continuation resume_with(Fn&& fn)&& {
	  ...
	  return { ontop_fcontext(std::exchange(fctx_, nullptr), &p, ctx_ontop< continuation, Fn >).fctx };
	}
  ...
  continuation callcc(std::allocator_arg_t, fixedsize_stack&& salloc, Fn&& fn) {
	return continuation{ create_ctx1<record<continuation, Fn>>(std::forward<fixedsize_stack>(salloc), std::forward< Fn >(fn)) }.resume();
  }
  template<typename Fn>
  continuation callcc(std::allocator_arg_t, preallocated palloc, fixedsize_stack&& salloc, Fn&& fn) {
	return continuation{ create_ctx2<record<continuation, Fn>>(palloc, std::forward<fixedsize_stack>(salloc), std::forward<Fn>(fn)) }.resume();
  };
