...
#define RB_ROTATE_LEFT(head, elm, tmp, field) do { (tmp) = RB_RIGHT(elm, field);  if ((RB_RIGHT(elm, field) = RB_LEFT(tmp, field)) != NULL) {  RB_PARENT(RB_LEFT(tmp, field), field) = (elm); }  RB_AUGMENT(elm); if ((RB_PARENT(tmp, field) = RB_PARENT(elm, field)) != NULL) { if ((elm) == RB_LEFT(RB_PARENT(elm, field), field))  RB_LEFT(RB_PARENT(elm, field), field) = (tmp); else RB_RIGHT(RB_PARENT(elm, field), field) = (tmp);  } else (head)->rbh_root = (tmp);  RB_LEFT(tmp, field) = (elm); RB_PARENT(elm, field) = (tmp); RB_AUGMENT(tmp); if ((RB_PARENT(tmp, field))) RB_AUGMENT(RB_PARENT(tmp, field)); } while ( 0)
 
#define RB_ROTATE_RIGHT(head, elm, tmp, field) do {  (tmp) = RB_LEFT(elm, field); if ((RB_LEFT(elm, field) = RB_RIGHT(tmp, field)) != NULL) {  RB_PARENT(RB_RIGHT(tmp, field), field) = (elm);  }  RB_AUGMENT(elm); if ((RB_PARENT(tmp, field) = RB_PARENT(elm, field)) != NULL) { if ((elm) == RB_LEFT(RB_PARENT(elm, field), field))  RB_LEFT(RB_PARENT(elm, field), field) = (tmp); else RB_RIGHT(RB_PARENT(elm, field), field) = (tmp);  } else (head)->rbh_root = (tmp);  RB_RIGHT(tmp, field) = (elm);  RB_PARENT(elm, field) = (tmp); RB_AUGMENT(tmp); if ((RB_PARENT(tmp, field))) RB_AUGMENT(RB_PARENT(tmp, field)); } while ( 0)
