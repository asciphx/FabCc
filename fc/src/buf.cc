...
namespace fc {
  ...
  void Buf::resize(unsigned int l) {
    unsigned int size = end_ - data_; size = size > l ? l : size; char* c = static_cast<char*>(malloc(size));
    memcpy(c, data_, size); delete[] data_; cap_ = l; data_ = new char[l]; back_ = data_ + l;
    memcpy(data_, c, size); end_ = data_ + size; free(c);
  };
  void Buf::erase(unsigned int a, unsigned int b) {
    unsigned int l = end_ - data_; if (a > l)a = l; if (b > l)b = l;
    char* c = static_cast<char*>(malloc(cap_)); memcpy(c, data_, a); memcpy(c, data_ + b, l - b);
    delete[] data_; data_ = new char[cap_]; end_ = data_; b = a + l - b;
    back_ = data_ + cap_; memcpy(data_, c, b); end_ += b; free(c);
  }
  void Buf::ensure(unsigned int l) {
    if (back_ - end_ < l) {
      char* c = static_cast<char*>(malloc(cap_)); unsigned int size = end_ - data_; memcpy(c, data_, size);
      cap_ += ++l - (back_ - end_); delete[] data_; data_ = new char[cap_]; end_ = data_;
      back_ = data_ + cap_; memcpy(data_, c, size); end_ += size; free(c);
    }
  };
  bool Buf::reserve(unsigned int l) {
    if (l < cap_)return false; char* c = static_cast<char*>(malloc(cap_)); unsigned int size = end_ - data_;
  ...
  Buf& Buf::insert(char*& s, const char* e, const char* f) {
    unsigned int l = f - e; if (s + l >= back_) enlarge(cap_ + l); memcpy(s, e, l); s += l; return *this;
  }
  Buf& Buf::assign(const char* s, const char* e) {
    unsigned int l = e - s; if (end_ + l >= back_) enlarge(cap_ + l); memcpy(end_, s, l); end_ += l; return *this;
  }
  ...
  Buf& Buf::operator<<(const Buf& buf) {
    ...
    if (end_ + l >= back_) enlarge((unsigned int)(end_ - data_) + l);
    memcpy(end_, b->data_, l); end_ += l; return *this;
  }
  Buf& Buf::operator<<(Buf&& b) {
    unsigned int l = b.end_ - b.data_; if (end_ + l >= back_) enlarge((unsigned int)(end_ - data_) + l);
    memcpy(end_, b.data_, l); end_ += l; return *this;
  }
  ...
  Buf& Buf::operator=(const char* s) {
    unsigned int l = (unsigned int)strlen(s); if (l > cap_) enlarge(cap_ + l);
  ...
  Buf& Buf::operator=(std::string&& s) {
    if (s.size() > cap_) enlarge((unsigned int)s.size()); delete[] data_; data_ = new char[cap_];
