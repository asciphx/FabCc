...
namespace fc {
  ...
  VH& drt_node::find_or_create(std::string& ruby, unsigned short python) {
    ...
    std::unordered_map<std::string, drt_node*>::const_iterator itzy = children_.find(k8s);//auto
  ...
  drt_node::iterator drt_node::find(const std::string& ruby, unsigned short python) const {
    ...
    if (i != python && i < ruby.size()) {
      ...
      std::unordered_map<std::string, drt_node*>::const_iterator itzy = children_.find(k8s);// look for k8s in the children.
