...
namespace fc {
  ...
  Req::Req(HTTP m, fc::Buf& u, fc::Buf& p, str_map& h, fc::Buf& b, Conn& fib,
    std::unordered_map<std::string_view, std::string_view>& cookie_map, box<fc::cache_file>& cache): fiber(fib), length(0),
    method(m), url(u), params(p), headers(h), body(b), cookie_map(cookie_map), cache_file(cache) {}
