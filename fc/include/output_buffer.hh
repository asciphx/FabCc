...
namespace fc {
  struct output_buffer {
    ...
    FORCE_INLINE void reset() { cursor_ = buffer_; }
    FORCE_INLINE std::size_t size() { return cursor_ - buffer_; }
    FORCE_INLINE void flush() { flush_(buffer_, int(size())); reset(); }
    ...
    FORCE_INLINE output_buffer& operator<<(const char* s) { return operator<<(std::string_view(s, strlen(s))); }
    FORCE_INLINE output_buffer& operator<<(char v) { cursor_[0] = v; ++cursor_; return *this; }
    FORCE_INLINE output_buffer& operator<<(size_t& v) {
      if (v == 0) operator<<('0'); char c[20]; return operator<<(std::string_view(c, jeaiii::to_text_from_integer(c, v) - c));
    }
    FORCE_INLINE output_buffer& operator<<(size_t&& v) {
      if (v == 0) operator<<('0'); char c[20]; return operator<<(std::string_view(c, jeaiii::to_text_from_integer(c, std::move(v)) - c));
    }
    FORCE_INLINE output_buffer& append(const char c) { return (*this) << c; }
    template <typename I, std::enable_if_t<std::is_fundamental<I>::value>* = nullptr>
    FORCE_INLINE output_buffer& operator<<(I& v) {
    ...
    FORCE_INLINE std::string_view to_string_view() { return std::string_view(buffer_, cursor_ - buffer_); }
    char* buffer_;
    bool own_buffer_;
    char* cursor_;
    char* end_;
