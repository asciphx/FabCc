...
namespace fc {
  ...
  struct Reactor {
	...
	inline co& fd_to_fiber(socket_type fd) {
	...
	inline void reassign_fd_to_fiber(socket_type fd, socket_type fiber_idx) { fd_to_fiber_idx[fd] = fiber_idx; }
	inline void epoll_ctl(epoll_handle_t epoll_fd, socket_type fd, int action, socket_type flags) {
#if __linux__ || _WIN32
	  epoll_event event;
...
#if _WIN32
	inline void epoll_add(socket_type new_fd, int flags, socket_type fiber_idx = INVALID_SOCKET) {
	  epoll_ctl(epoll_fd, new_fd, EPOLL_CTL_ADD, flags);
#elif __linux__
	inline void epoll_add(socket_type new_fd, int flags, socket_type fiber_idx = -1) {
	  epoll_ctl(epoll_fd, new_fd, EPOLL_CTL_ADD, flags);
#elif __APPLE__
	inline void epoll_add(socket_type new_fd, int flags, socket_type fiber_idx = -1) {
	...
	inline void epoll_del(socket_type fd) {
...
#elif _WIN32
		  if (event_flags & (EPOLLERR | EPOLLRDHUP)) {
#elif __APPLE__
  ...
  inline void start_tcp_server(std::string ip, int port, int socktype, int nthreads, std::function<void(Conn&)> conn_handler,
