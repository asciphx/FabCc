...
namespace fc {
  struct Ctx {
    Ctx(input_buffer& _rb, Conn& _fiber): rb(_rb), fiber(_fiber) {
      ...
      output_stream = output_buffer(32768, [&](const char* d, int s) { fiber.write(d, s); });
      headers_stream = output_buffer(996, [&](const char* d, int s) { output_stream << std::string_view(d, s); });
    }
