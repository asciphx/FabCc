...
struct input_buffer {
  ...
  input_buffer() : buffer_(65536), cursor(0), end(0) {}
  // Free unused space in the buffer in [i1, i2[.
  // This may move data in [i2, end[ if needed.
  void free(int i1, int i2) {
    assert(i1 < i2);
    assert(i1 >= 0 && i1 < buffer_.size());
    assert(i2 > 0 && i2 <= buffer_.size());
    if (i1 == cursor && i2 == end) // eat the whole buffer.
      cursor = end = 0;
  ...
  template <typename F> std::string_view read_more_str(F& fiber) {
    int l = read_more(fiber); return std::string_view(buffer_.data() + end - l);
  }
  ...
  template <typename F> std::string_view read_until(F&& fiber, const char*& start, char delimiter) {
    ...
    while (true) {
      ...
      while (str_end < buffer_end && *str_end != delimiter)
        str_end++;
