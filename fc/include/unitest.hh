...
#define DEF_test($) \
    ...
    static unitest::TestSaver _UT_sav_test_##$(str::make<_UTest_##$>()); \
    void _UTest_##$::run()
// define a test case in the current unit
#define DEF_case(name) _current_case.reset(str::make<unitest::Case>(#name));
#define EXPECT(x) \
{ \
    if (_current_case == NULL) { \
        _current_case.reset(str::make<unitest::Case>("default")); \
    } \
    if (x) { \
