...
#endif
#define TL_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)                                     \
    std::detail::is_trivially_copy_constructible<T>::value
...
#define TL_TRAITS_LIBCXX_MEM_FN_WORKAROUND
	...
	struct invoke_result_impl<
	  F, decltype(std::invoke(std::declval<F>(), std::declval<Us>()...), void()),
	  Us...> {
	  using type = decltype(std::invoke(std::declval<F>(), std::declval<Us>()...));
	};
	template <class F, class... Us>
	...
	// Trait for checking if a type is a std::optional
  ...
  /// object if there is one.
	...
	constexpr optional(nullptr_t) noexcept: m_value(nullptr) {}
	/// Copy constructor
	///
	/// If `rhs` contains a value, the stored value is direct-initialized with
	/// it. Otherwise, the constructed optional is empty.
