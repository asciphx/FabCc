...
namespace ctx {
  ...
  fcontext_t add_ctx(fixedsize_stack&& salloc, Fn&& fn) {
    ...
    Record* record = new (storage) Record{ sctx, std::forward<fixedsize_stack>(salloc), std::forward<Fn>(fn) };
  ...
  private:
    ...
    template<typename Fn> friend co callcc(Fn&&);
  ...
  public:
    ...
    template<typename Fn, typename = std::disable_overload<co, Fn>>
    co(Fn&& fn) : co{ fixedsize_stack(CTX_MIN_SIZE), std::forward<Fn>(fn) } {}
  ...
  template<typename Fn>
  _FORCE_INLINE co callcc(Fn&& f) { return co{ add_ctx<record<co, Fn>>(fixedsize_stack(CTX_MIN_SIZE), std::forward<Fn>(f)) }.resume(); };
  typedef co fiber;
}
