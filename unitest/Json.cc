...
namespace test {
  DEF_test(json) {
	...
	DEF_case(string) {
	  ...
	  EXPECT_EQ(s.str(), fc::Buf("\"").append(30, 'x').append("\\n").append(500, 'x').append('"'));
	  EXPECT_EQ(s.dbg(), fc::Buf("\"").append(30, 'x').append("\\n").append(1, 'x').append(3, '.').append('"'));
	  s = std::string(600, 'x');
	  EXPECT_EQ(s.str(), fc::Buf("\"").append(600, 'x').append('"'));
	  EXPECT_EQ(s.dbg(), fc::Buf("\"").append(32, 'x').append(3, '.').append('"'));
	...
	DEF_case(parse_int) {
	  ...
	  fc::Buf s("12345678901234567890888");
	  s.resize(3);
	...
	DEF_case(parse_string) {
	  ...
	  v = json::parse(fc::Buf().append('"').append(300, 'x').append('"'));
	  EXPECT(v.is_string());
	  EXPECT_EQ(v.str(), fc::Buf().append('"').append(300, 'x').append('"'));
	}
	...
	DEF_case(parse_object) {
	  ...
	  fc::Buf ss = "{ \"hello\":23, \"world\": { \"xxx\": 99 } }";
	  v = json::parse(ss.data_, ss.size());
	  EXPECT(v.is_object());
