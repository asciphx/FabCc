...
int main() {
  ...
  app["/params"] = [](Req& req, Res& res) -> Task<int> {
    std::string s;
  ...
  app.default_route() = [](Req& req, Res& res) -> Task<int> {
  ...
  app["/get_upload"] = [](Req& req, Res& res) -> Task<int> {
    res.write_async([] {
  ...
  app["/read_file"] = [](Req& req, Res& res) -> Task<int> {
    res.write_async([] { Json x = json::read_file("test.json"); return x; }); _return
  };
  app["/json"] = [](Req& req, Res& res) -> Task<int> {
  ...
  app["/serialization"] = [](Req& req, Res& res) -> Task<int> {
    Json x = json::parse(R"(
    ...
    )"); Book b = x.get<Book>(); b.person->book->persons[2].name = "wwzzgg"; x = b; res.write(x.dump()); _return
  };
  app["/api"] = [](Req& req, Res& res) -> Task<int> {
    res.write(res.app._print_routes()); _return//Return to routing list
  };
  app.post("/api") = [](Req& req, Res& res) -> Task<int> {
  ...
  app["/del"] = [&app](Req&, Res& res) -> Task<int> { app["/"] = nullptr; res.write("The routing of the home page is delete！！"); _return };
  app["/timer"] = [](Req& req, Res& res) -> Task<int> {
    req.setTimeout([] { raise(SIGINT); }, 6000);
    res.write("Turn off the server timer and start the countdown！"); _return
  };
